...
public class TextTranslationServiceTests
{
    ...
    {
        var result = await _textTranslationService.TranslateAndFormatAsync("", "en", "es");
    ...
    public async Task TranslateAndFormatAsync_ShouldPreserveFormatting()
    {
        ...
        _translatorMock.Setup(t => t.TranslateAsync("Welcome to translation!", "en", "es"))
            .ReturnsAsync(new ApiResponse<string> { Data = "¡Bienvenido a la traducción!", Successful = true });

        // Act
        var result = await _textTranslationService.TranslateAndFormatAsync(input, "en", "es");
    ...
    public async Task TranslateAndFormatAsync_ShouldHandleLinks()
    {
        ...
        _translatorMock.Setup(t => t.TranslateAsync("https://example.com", "en", "es"))
        ...
        // Assert
        result.Should().Be("\thttps://ejemplo.com\n");
    }
    ...
    {
        // Arrange
        _translatorMock.Setup(t => t.GetSupportedLanguagesAsync()).ReturnsAsync(new string[] { "es", "fr" });

        // Act
        var result = await _textTranslationService.TranslateFolder("test_folder", "de");

        // Assert
        result.Successful.Should().BeFalse();
        result.Message.Should().Be("Language not supported");
    }
}